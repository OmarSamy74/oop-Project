#include "Event.h"
class Event{

    private:

    int choice;
    Guess_Game player;
    TicTacToe game;
    Menu menu;
    string recommendation;

    public:

    void rush(){

        cout << "Succesfully stated being in a rush.\n Extra fees will be applied.\n";
        // add code to increase total order price by 5%

    }

    void clearTerminal() {

        #ifdef _WIN32           /* # to define a preprocessor directive (processed before actual code)
                                    if the _WIN32 macro is defined --> use cls*/
            system("cls");
        #else
            system("clear");    // For other os
        #endif
        }

    void startup() {

        cout << "Welcome to Baladina!\nOur services:\n"
            "1. Credit card services.\n"
            "2. Let us get to know you.\n"
            "3. Edit personal information\n"
            "4. Order.\n"
            "Please enter the number of your desired service or 'q' to quit: ";
    }

    void save_data(vector<Customer>customers) {
        //open file in write mode
        ofstream outfile("customer_data.txt");
        //test file opening
        if (!outfile) {
            cerr << "Error opening file for writing\n";
            return;
        }
        // check each customer in vector
        for (Customer customer : customers) {        // range based loop
            // write each customer component
            outfile << "Name: " << customer.getName() << endl;
            outfile << "Age: " << customer.getAge() << endl;
            outfile << "Gender: " << customer.getGender() << endl;
            outfile << "ID: " << customer.getID() << endl;

            vector<double> loyalty_points = customer.getLoyaltyPoints();
            // check if loyalty points are empty
            if (!loyalty_points.empty()) {
                outfile << "Loyalty Points: ";
                outfile << endl;
                }

            outfile << endl; // write new line after each customer
        }

            outfile.close();
    }

    void waiting() {


            do {

                cout << "\n"
                    << "Enter your choice:\n"
                    << "\n"
                    << "1. Guess Game\n"
                    << "2. Tic-Tac-Toe\n"
                    << "3. Are you in a rush?\n"
                    << "4. Any menu items recommendations?\n"
                    << "5. Wait the remaining time.\n";

                    cin >> choice;

                cin.clear();
                fflush(stdin);

                switch (choice) {

                case 1:

                    player.playQuiz();

                case 2:

                    game.playTicTacToe();

                case 3:

                    rush();

                case 4:

                    cout << "Enter your menu recommendation (one item per line, type 'done' to finish):\n";

                    while (true) {
                        cin >> recommendation;
                        //convert to uppercase
                        for (char &c : recommendation) {
                            c = toupper(c);
                        }

                        if (recommendation == "DONE") 
                            break;
                    }
                    cout << "Thank you for your recommendation.\n";
                    menu.addRecommendation(recommendation);

                case 5:
                    cout << "Thanks for visiting!\n";
                    break;

                default:
                    cout << "Invalid choice\n";
                }

            } while (choice != 5);
        }
    };